...
class SamplesOscillogram:
    ...
    def __init__(self, master, file, filetype):
        ...
        self.__class__.figure.canvas.set_window_title(f'Осциллограмма файла \'{self.__class__.fileName}\'')
    ...
    def updatePlot(self):
        ...
        while True:
            ...
            x = np.linspace(samplesNum * packetsPlotted, samplesNum * (packetsPlotted + 1), samplesNum)
            x_Interp1d = np.linspace(samplesNum * packetsPlotted, samplesNum * (packetsPlotted + 1), samplesNum*5)
            A_Interp1d = interp1d(x, A, kind='cubic')
            #self.__class__.subplotMain.plot(x, A)
            self.__class__.subplotMain.plot(x_Interp1d, A_Interp1d(x_Interp1d))
            packetsPlotted += 1
        # Configuring y axes
        plt.setp(self.__class__.subplotMain, ylim=(self.__class__.yLimitLow, self.__class__.yLimitHigh))
        return 0
